...
<section class="mt-8" x-data="{ tab: 'all' }">
    <div class="flex items-center justify-between gap-4">
        ...
        <a href="{{ route('submissions.index') }}" class="text-sm font-medium text-red-700 hover:text-red-800">View all &rarr;</a>
    </div>
                ...
                @forelse ($data['submissions'] as $submission)
                    <tr x-show="tab === 'all' || tab === '{{ $submission->status->value }}'">
                        ...
                        <td class="whitespace-nowrap px-4 py-3 text-right">
                            <a href="{{ route('submissions.show', $submission) }}" class="text-sm font-medium text-red-700">View &rarr;</a>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-10 text-center text-slate-500">
                            ...
                            @if (auth()->user()->isApproved())
                                <a href="{{ route('submissions.create') }}" class="mt-3 inline-flex rounded-xl bg-red-700 px-4 py-2 text-sm font-medium text-white hover:bg-red-800">+ Create New Submission</a>
                            @endif
                        </td>
                    </tr>
                @endforelse
