            ...
            <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
                <table class="min-w-full divide-y divide-slate-200 text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($logs as $log)
                            @php
                                $statusLower = str($log->statusLabel())->lower();
                                $statusClasses = match (true) {
                                    $statusLower->contains(['approved', 'created', 'assigned']) => 'bg-emerald-100 text-emerald-700',
                                    $statusLower->contains(['revision', 'required']) => 'bg-amber-100 text-amber-700',
                                    $statusLower->contains(['deleted', 'removed', 'disabled']) => 'bg-rose-100 text-rose-700',
                            ...
                            <tr>
                                ...
                                <td class="whitespace-nowrap px-4 py-3 text-slate-600">{{ $log->causer?->role?->label() ?? '—' }}</td>
                                <td class="whitespace-nowrap px-4 py-3">
                                    <span class="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-600">{{ $log->activityLabel() }}</span>
                                </td>
                                <td class="whitespace-nowrap px-4 py-3">
                                    <span class="rounded-full px-2.5 py-1 text-xs font-medium {{ $statusClasses }}">{{ $log->statusLabel() }}</span>
                                </td>
                                <td class="px-4 py-3 text-slate-700">{{ $log->description }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-6 text-center text-slate-500">No activity recorded yet.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
