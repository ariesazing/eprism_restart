...
<div class="mt-6 min-w-0 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200" x-data="{ enabled: {{ ($mf['enabled'] ?? false) ? 'true' : 'false' }} }">
    <div class="flex items-start justify-between gap-4">
        <div>
            <h3 class="text-sm font-semibold text-slate-900">Manuscript formatting</h3>
            <p class="mt-1 text-xs text-slate-500">
                Controls how a generated submission's manuscript is formatted &mdash; body text, headings,
                tables, captions and page margins. Applied automatically when a manuscript is generated;
                anything left blank keeps whatever formatting is already in this document.
            </p>
        </div>
    ...
    <form id="manuscript-format-form" method="POST" action="{{ route('admin.document-templates.manuscript-format.update', $templateKey) }}" class="mt-4 space-y-6" :class="{ 'opacity-50': !enabled }">
        @csrf
        ...
        {{-- Tables --}}
        <fieldset class="rounded-xl border border-slate-200 p-4" x-data="{ inherit: {{ ($table['inherit'] ?? true) ? 'true' : 'false' }} }">
            ...
            <div class="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3" :class="{ 'opacity-50': inherit }">
                <label class="text-xs text-slate-600">Font
        ...
        {{-- Captions --}}
        <fieldset class="rounded-xl border border-slate-200 p-4" x-data="{ inherit: {{ ($caption['inherit'] ?? true) ? 'true' : 'false' }} }">
        ...
        <div class="flex items-center gap-3">
            <button type="submit" class="rounded-lg bg-cherry-700 px-4 py-2 text-sm font-semibold text-white hover:bg-cherry-800">Save manuscript formatting</button>
            @if ($hasManuscriptDocx)
                <button type="submit" formaction="{{ route('admin.document-templates.manuscript-format.preview', $templateKey) }}" formtarget="_blank" class="rounded-lg bg-white px-4 py-2 text-sm font-semibold text-slate-700 ring-1 ring-slate-300 hover:bg-slate-50">Preview formatting</button>
            @else
