<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            ...
            <a href="{{ route('admin.rapm-templates.index') }}" class="text-sm font-medium text-cherry-700">Back to templates</a>
        </div>
    </x-slot>
    ...
    <div class="py-10">
        <div class="mx-auto grid max-w-6xl gap-6 px-4 sm:px-6 lg:px-8 lg:grid-cols-[1fr,320px]">
            ...
            <form method="POST" action="{{ route('admin.rapm-templates.update', $templateKey) }}" class="min-w-0 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
                @csrf
                ...
                <div class="mt-4 flex items-center gap-3 border-t border-slate-100 pt-4">
                    ...
                    @if ($hasPreviewSubmission)
                        <button
                            type="submit"
                            formaction="{{ route('admin.rapm-templates.preview', $templateKey) }}"
                            formtarget="_blank"
                    ...
                    @else
                        <span class="text-xs text-slate-400">Preview unavailable &mdash; no submission exists yet.</span>
                    @endif
                </div>
            </form>
