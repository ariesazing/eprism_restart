<x-app-layout>
    <x-slot name="header">
        <div>
            <h2 class="text-xl font-semibold leading-tight text-slate-800">RAPM Document Templates</h2>
            <p class="mt-1 text-sm text-slate-500">Edit the Review Summary and Routing Slip templates automatically generated during the review/approval process.</p>
        </div>
    </x-slot>
                ...
                <div class="mt-3 grid gap-4">
                    @foreach ($templates as $template)
                        <div class="flex flex-col gap-3 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200 md:flex-row md:items-center md:justify-between">
                            ...
                            <a href="{{ route('admin.rapm-templates.edit', $template['key']) }}" class="rounded-xl bg-cherry-700 px-4 py-2 text-center text-sm font-medium text-white hover:bg-cherry-800">Edit Template</a>
                        </div>
                    @endforeach
                </div>
            </section>
