...
| **Check grammar button** | On the chapters page for ONLYOFFICE chapters (which can't underline grammar live), a button opens a read-only view of the chapter with grammar/spelling issues highlighted and suggestions on hover. Needs the LanguageTool service you already run (`LANGUAGETOOL_URL`); if it's unreachable the view says so. The readiness (SRAM) grammar score now also reads the saved chapter text instead of a lagging copy. |
